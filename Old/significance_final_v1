...
def p_value_graph (statDict):
    #key: graphname, value: nber of times coputation is higher than in original induced graph
    p_value = []
    
    return  p-value


def writeDict(statDict, stat_filename, sep):
    with open(stat_filename, "a") as f:
        for i in statDict.keys():
            print (" statDict[i][0]: ", statDict[i][0])
